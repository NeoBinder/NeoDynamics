...
here once, kernel-agnostic on the :class:`~neomd2.kernel.port.KernelPort` seam
...
modulo checks) but the :class:`~neomd2.probes.RunView` is constructed only
...
Progress logging goes to ``logging.getLogger("neomd2.driver")``; the
...
LOG = logging.getLogger("neomd2.driver")
...
def run_md(
    ...
    *,
    ...
    view:        factory ``(kernel, step) -> RunView``; the default wraps the
                 kernel in a :class:`~neomd2.probes.KernelView` with the
...
def _append_restraint_probe(probes: list, plan, sink, kernel, fgroups) -> None:
    """Append the :class:`~neomd2.probes.RestraintProbe` the plan's derived
...
def drive(
    ...
    * method ``"min"`` → :func:`run_minimization`; ``"eq"``/``"md"``/
      ...
      :class:`~neomd2.probes.RestraintProbe` when the derived
    ...
    * a :class:`~neomd2.manifest.RunManifest` opens epoch 0 ("start") before
    ...
    if restraint:
        from . import registry
        import neomd2.restraints  # noqa: F401  (import = triple registration)
    ...
    if method in _MIN_METHODS:
        ...
        # registration (importing neomd2.methods registers its entries).
