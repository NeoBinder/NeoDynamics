...
``neomd2 = neomd2.cli:main`` is the console-script spelling registered in
...
mapping onto a public library call —
    ...
    run      DIR | plan.yaml [--steps N] [--platform cpu|cuda]
             [--kernel openmm|fake|replay]
                  -> neomd2.md_run(target, platform=..., kernel=...,
    ...
    prepare  config.yaml
                  -> neomd2.system.prepare_system(yaml.safe_load(...)) + a
    ...
    version       -> neomd2.__version__

Exit codes: 0 success; 2 user error — the :class:`~neomd2.errors.NeoUserError`
...
``run --kernel replay`` imports :mod:`neomd2.kernel.replay` before calling
...
def build_parser() -> argparse.ArgumentParser:
    ...
    version = sub.add_parser("version", help="print the neomd2 version")
...
def _tool_main(command: str):
    ...
    test_migrate.py): nothing under src/neomd2 may statically reference the
    ...
    tool is gone, ``neomd2 migrate`` prints a clean not-available message
...
def _cmd_migrate(args) -> int:
    ...
    if tool is None:
        print("neomd2 migrate: the v1 -> v2 translator is not available "
...
def _cmd_prepare(args) -> int:
    ...
    try:
        ...
        print(f"neomd2 prepare: cannot read {args.config!r}: {error}",
              file=sys.stderr)
        return 2
...
def main(argv=None) -> int:
    """``neomd2 <command> ...`` — see the module docstring for the surface.
...
if __name__ == "__main__":  # python -m neomd2.cli
    sys.exit(main())
