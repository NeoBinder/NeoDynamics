...
* v1's ``GAFFTemplateGenerator`` subclass of openmmforcefields (the
  ...
  :class:`~neomd2.tools.port.ChargeBackend` / ``ParamBackend`` protocols,
  executing antechamber/parmchk2 through a :class:`~neomd2.tools.port.ToolRunner`
...
* v1's ``ComplexForceField`` (force field assembly + ``sys_params_from_config``
  ...
  seam consumed by ``neomd2.system``.

Unit convention (deliberate simplification, documented): partial charges are
plain floats in *elementary charge* everywhere inside neomd2 ("elementary
...
_logger = logging.getLogger("neomd2.tools.antechamber")
...
class AntechamberBackend:
    ...
    runner:
        ...
        :class:`~neomd2.tools.port.FakeToolRunner` in tests, a
...
def build(topology, positions=None, ligands=None, ff_kwargs=None, sys_kwargs=None,
    ...
    played.  Signature-compatible with the ``neomd2.system`` seam
