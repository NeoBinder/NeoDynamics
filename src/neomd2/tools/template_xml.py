...
``yaml.safe_load`` into the plain-dict config convention used across neomd2):
...
``generate_template``
    ...
    :class:`~neomd2.tools.antechamber.AntechamberBackend.generate_residue_template`
...
from neomd2.tools.antechamber import AntechamberBackend
from neomd2.tools.ligand import ligands_from_config
from neomd2.tools.port import SubprocessToolRunner, ToolRunner
...
def generate_template(config, *, runner: ToolRunner | None = None) -> str:
    ...
    ``runner`` selects the :class:`~neomd2.tools.port.ToolRunner` executing
    antechamber/parmchk2 (a ``FakeToolRunner`` in tests; a
    :class:`~neomd2.tools.port.SubprocessToolRunner` by default).  Returns
...
def _load_config(path):
    """v1's ``Box.from_yaml(filename=...)`` as the neomd2 plain-dict read."""
    import yaml
