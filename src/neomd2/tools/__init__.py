"""neomd2.tools — the external-tool seam (v2 migration plan §5 item 2.4).

* :mod:`neomd2.tools.port` — ``ToolRunner`` (isolated execution +
  diagnostics), ``ToolResult`` / ``ToolError``, the in-process
  ``FakeToolRunner``, and the minimal ``ChargeBackend`` / ``ParamBackend``
  protocols.
* :mod:`neomd2.tools.antechamber` — the GAFF/antechamber knowledge ported
...
Later: ``neomd2.tools.orca`` (resp2_orca workflow, plan item 2.5).
"""

from neomd2.tools.antechamber import (
    AntechamberBackend,
