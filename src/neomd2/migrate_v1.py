...
(tests/v2/test_migrate.py) and ``python -m neomd2.migrate_v1``.  When the
...
def _key_lines(source) -> dict:
    ...
    Same YAML-composition walk ``neomd2.plan._yaml_line_map`` does for plan
...
def main(argv=None) -> int:
    """``python -m neomd2.migrate_v1 input.yaml [-o out.yaml] [--base-dir DIR]
    [--dry-run]``.
    ...
    parser = argparse.ArgumentParser(
        prog="python -m neomd2.migrate_v1",
        description=(
            "One-shot translator: v1 run-config YAML -> neomd2 plan YAML "
