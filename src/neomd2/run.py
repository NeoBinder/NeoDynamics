"""run — the C facade of neomd2 (v2 migration plan §2, §5 item 1.6).
...
:class:`~neomd2.kernel.port.KernelFactory`, and wraps everything in a
:class:`~neomd2.sinks.LocalDirSink` on ``plan.output_dir`` plus
:func:`neomd2.driver.drive`.  A :class:`CompiledRun` is a handle, not a
runner: ``.run()`` delegates to the driver and returns its
:class:`~neomd2.driver.RunOutcome`.
...
* ``compile(kernel="fake")`` raises :class:`NotImplementedError`: the fake
  ...
  ``neomd2.driver.drive(plan, kernel_factory=...)`` — the driver tests do
  exactly that.
...
PLAN_FILENAMES = ("neomd2.yaml", "plan.yaml", "neomd2.json", "plan.json")
...
def _discover_plan_file(directory: str) -> str:
    ...
    if len(found) > 1:
        raise ConfigKeyError(
            ...
            f"neomd2.yaml or pass the file path directly",
            candidates=found,
            known_keys=PLAN_FILENAMES,
        )
...
def _kernel_spec(plan: Plan, *, kind: str = "openmm", platform: str = "cpu"):
    """Compile the plan into a :class:`~neomd2.kernel.port.KernelSpec`.
...
class CompiledRun:
    ...
    :meth:`run` delegates to :func:`neomd2.driver.drive`, which owns restraints,
...
def compile(plan_or_dict, *, kernel: str = "openmm", platform: str = "cpu",
    ...
    ``plan_or_dict``: a :class:`~neomd2.plan.Plan` or the plan dict (validated
    ...
    if kernel == "fake":
        raise NotImplementedError(
            ...
            "neomd2.driver.drive(plan, kernel_factory=...) "
...
def md_run(target, *, platform: str = "cpu", kernel: str = "openmm",
    ...
    **overrides:
        ...
        raise :class:`~neomd2.errors.ConfigKeyError` with a did-you-mean).

    Returns the :class:`~neomd2.driver.RunOutcome` of the run.
    """
