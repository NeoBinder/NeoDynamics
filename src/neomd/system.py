...
* :class:`SystemBundle` — pure data + loading + validation.  It NEVER imports
  ...
  turns the same IR into a :class:`~neomd.kernel.port.KernelSpec`; the
...
``neomd.tools.antechamber``.  :func:`prepare_system` takes that layer as a
...
unless ``neomd.tools.antechamber`` is importable and exposes a builder;
``neomd.tools`` is imported lazily inside the default paths and its absence
...
def _openmm():
    ...
    try:
        ...
        raise ImportError(
            "this neomd.system workflow requires openmm "
