...
当前提供:
  ...
  - seq       : 从 PDB 坐标 (ATOM/HETATM) 记录提取蛋白序列, 写出 fasta。
                (文本级, 不依赖 MDAnalysis; 不读取 SEQRES 头部)
...
用法示例:
  ...
  # 从多个 PDB 的坐标记录提取蛋白序列, 合并写出 fasta
...
def _extract_sequence_from_atoms(atoms, chain=None):
    """从 ATOM/HETATM 行 (坐标部分) 按出现顺序提取蛋白序列。

    按 (chainID, resSeq, iCode) 去重, 每个残基取第一个原子的 resName,
    映射为一字母代码。chain 非 None 时只保留该链。
    返回 (seq_str, chain_id, n_skipped_unknown)。
    注意: 不读取 SEQRES 头部记录, 只基于坐标行。
    """
    chain_set = None
    if chain:
        chain_set = {c.strip() for c in str(chain).split(",") if c.strip()}
    ...
    for ln in atoms:
        ch = ln[21]
        if chain_set is not None and ch not in chain_set:
            continue
        key = (ch, ln[22:26], ln[26])  # (chainID, resSeq, iCode)
        if key in seen:
            continue
        seen.add(key)
        ...
        if ch not in chain_ids:
            chain_ids.append(ch)
    return "".join(seq_chars), chain_ids, unknown


def extract_sequence(args):
    """提取一个或多个 PDB 的蛋白序列 (基于坐标 ATOM 记录), 写出 fasta。

    fasta header 默认为 PDB 文件名 (去扩展名); --id-format 可加链 id。
    序列按 --width 折行 (默认 60, 0 表示不折行)。多链时默认合并所有链;
    用 --chain 限定单链。
    """
    records = []
    for path in args.input:
        atoms, _, _ = _read_pdb(path)
        seq, chain_ids, unknown = _extract_sequence_from_atoms(
            atoms, chain=args.chain
        )
        ...
        if args.id_format == "file":
            ...
            header = f"{stem}|chain_{''.join(chain_ids)}"
        records.append((header, seq))
        print(f"{stem}: {len(seq)} 残基 (链 {','.join(chain_ids)})",
              file=sys.stderr)
    ...
    if args.output:
        with open(args.output, "w") as f:
            f.write(text)
        print(f"已写入 {len(records)} 条序列 -> {args.output}",
              file=sys.stderr)
    else:
        print(text, end="")
...
def main():
    ...
    p_seq = subparsers.add_parser(
        "seq",
        help="从 PDB 坐标 (ATOM/HETATM) 记录提取蛋白序列, 写出 fasta (文本级)"
    )
    p_seq.add_argument("-i", "--input", required=True, nargs="+",
                       help="输入 PDB 文件, 可多个 (如 *.pdb)")
    p_seq.add_argument("--chain", default=None,
                       help="只提取指定链 id (如 'A'); 不指定则合并所有链")
